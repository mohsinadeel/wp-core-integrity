...
class WP_Core_Integrity
{
    ...
    public function check_wp_integrity()
    {
        ...
        echo '<h3>Details:</h3>';
        echo '<p><strong>Total Files Checked: </strong>' . $total_files_checked
             . '<p>';
        echo '<p><strong>Total Time Taken: </strong>' . ($end_time
                                                         - $start_time)
             . ' seconds<p>';
        echo '</div>';
        echo '</div>';
    ...
    public function check_files_changes_in_core_files()
    {
        ...
        $checksums = array_intersect_key($json['checksums'],
            array_flip(preg_grep('/^wp-content.+/',
                array_keys($json['checksums']), PREG_GREP_INVERT)));
        ...
        foreach ($checksums as $file => $checksum) {
            ...
            if (file_exists($file_path)) {
                if (md5_file($file_path) !== $checksum) {
                    ...
                    $errors[]    = '<strong>HELP!</strong> Checksum for : '
                                   . $file_path . ' does not match!';
                    $hasNoErrors = false;
                }
            } else {
